...
function initializeDataGrid() {
    console.log('Initializing Subscriptions DataGrid...');

    try {
        $("#subscriptionsDataGrid").dxDataGrid({
            dataSource: {
                load: function () {
                    console.log('Loading subscriptions...');
                    return $.ajax({
                        url: getUrl('GetSubscriptions'),
                        type: 'GET',
                        dataType: 'json'
            ...
            columns: [
                ...
                {
                    dataField: 'DurationInDays',
                    caption: 'Duration',
                    width: 120,
                    alignment: 'center',
                    cellTemplate: function(container, options) {
                        const duration = options.value;
                        const text = duration === 1 ? '4 hours' : `${duration} days`;
                        $('<span>').text(text).appendTo(container);
                    }
                },
                {
                    dataField: 'MaxFileUploads',
                    caption: 'Max Files',
                    width: 110,
                    alignment: 'center',
                    cellTemplate: function(container, options) {
                        const text = options.value === 0 ? 'Unlimited' : options.value;
                        $('<span>').text(text).appendTo(container);
                    }
                },
                {
                    dataField: 'HasUnlimitedAccess',
                    caption: 'Full Access',
                    width: 120,
                    alignment: 'center',
                    cellTemplate: function(container, options) {
                        const badgeClass = options.value ? 'badge bg-success' : 'badge bg-warning';
                        const text = options.value ? 'Yes' : 'Limited';
                        $('<span>')
                            .addClass(badgeClass)
                            .text(text)
                            .appendTo(container);
...
function toggleStatus(data) {
    ...
    DevExpress.ui.dialog.confirm(
        `Are you sure you want to ${action} the subscription "${data.Name}"?`,
...
function confirmDelete(data) {
    ...
    DevExpress.ui.dialog.confirm(
        `Are you sure you want to delete the subscription "${data.Name}"?`,
        'Confirm Delete'
    ).done(function (dialogResult) {
        if (dialogResult) {
...
function refreshGrid() {
    ...
    $("#subscriptionsDataGrid").dxDataGrid('instance').refresh();
}

function getUrl(action) {
    const baseUrl = window.location.origin;
    return `${baseUrl}/Subscriptions/${action}`;
}
