$(document).ready(function () {
    initializeActivityLogsDataGrid();
});

function initializeActivityLogsDataGrid() {
...
showPageSizeSelector: true,
      ...
      buttons: [
 {
       hint: 'View Details',
         icon: 'info',
     onClick: function (e) {
 showAuditLogDetails(e.row.data.id);
      }
     }
           ]
            }
...
function showAuditLogDetails(logId) {
    $.ajax({
  ...
  width: 700
   ...
   console.error('Error loading audit log details:', error);
            toastr.error('Failed to load audit log details');
   }
    });
}
